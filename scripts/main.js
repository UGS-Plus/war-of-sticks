  ...
  SetPanXYA(a, b, c) {
    if (this._isPannerEnabled) {
      ...
      if (d[0] !== a || d[1] !== b || 0 !== d[2])
        (d[0] = a), (d[1] = b), (d[2] = 0), this._pannerNode.setPosition(...d);
...
function IsWebShareFilesSupported() {
  ...
  return (
    ...
    navigator.canShare({ files: [new a(["test file"], "test.txt", {})] })
  );
}
self.RuntimeInterface.AddDOMHandlerClass(
  class extends self.DOMHandler {
    constructor(a) {
      ...
      this._isSupported = "function" === typeof navigator.share || a;
      this._isFilesSupported = IsWebShareFilesSupported() || a;
      this.AddRuntimeMessageHandlers([
        ["init", (b) => this._OnInit(b)],
        ["share", (b) => this._OnShare(b)],
        ["request-rate", (b) => this._OnRateApp(b)],
        ["request-store", (b) => this._OnShowStore(b)],
      ]);
    }
    _OnInit() {
      return {
    ...
    _GetRatePlugin() {
      return (
        window.cordova &&
        window.cordova.plugins &&
        window.cordova.plugins.RateApp
      );
    }
    async _OnShare(a) {
      var b = a.text;
      ...
      if (f) {
        ...
        if (e) {
          b = 0;
          for (var g of a) b += g.size;
          try {
            const k = await this._CordovaGetTempDirEntry(Math.floor(1.25 * b));
            h.files = await Promise.all(
              a.map((l) => this._CordovaWriteTempFile(l, k))
            );
          } catch (k) {
            console.log("[Share plugin] Share failed: ", k);
        ...
        e && (g.files = a);
        try {
          await navigator.share(g), this.PostToRuntime("share-completed");
        } catch (h) {
          console.log("[Share plugin] Share failed: ", h),
            this.PostToRuntime("share-failed");
        }
      }
    }
    ...
    _CordovaGetTempDirEntry(a) {
      return new Promise((b, c) => {
        window.requestFileSystem(window.TEMPORARY, a, (d) => b(d.root), c);
      });
    }
...
self.RuntimeInterface.AddDOMHandlerClass(
  class extends self.DOMHandler {
    ...
    GameLoadingFinished() {
      this._pokiSDKLoaded &&
        !this._finishedLoadingSent &&
        ((this._finishedLoadingSent = !0), PokiSDK.gameLoadingFinished());
    }
    NotifyGameplayStart() {
      this._pokiSDKLoaded &&
        (this._finishedLoadingSent ||
    ...
    NotifyGameplayStop() {
      this._pokiSDKLoaded &&
        ((this._gameplayActive = !1), PokiSDK.gameplayStop());
    }
    HappyTime({ intensity: a }) {
      this._pokiSDKLoaded && PokiSDK.happyTime(a);
    }
    RequestCommercialBreak() {
      if (!this._pokiSDKLoaded) return { result: !1, err: !1 };
      ...
      return PokiSDK.commercialBreak()
        .then(() => {
          ...
          return { result: !0, err: !1 };
        })
        .catch((a) => {
          console.error(a);
          return { result: !1, err: a };
        });
    }
    ...
    InitPoki({ debugMode: a, preventScroll: b }) {
      if ("undefined" !== typeof PokiSDK) {
        ...
        return PokiSDK.init()
          .then(() => {
            ...
            return { loaded: this._pokiSDKLoaded, adBlock: c };
          })
          .catch(() => {
      ...
      return Promise.resolve({ loaded: this._pokiSDKLoaded, adBlock: !1 });
    }
    preventScroll() {
